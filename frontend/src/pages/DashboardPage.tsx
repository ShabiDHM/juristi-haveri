...
// PHOENIX PROTOCOL - DASHBOARD V32.5 (ICON USAGE & TS COMPLIANCE)
// 1. FIX: Integrated 'User' icon into Representative section to resolve TS6133 (unused import).
// 2. UI: Enhanced visual hierarchy of Create Client modal with dual iconography (Building vs User).
// 3. STATUS: 100% Type Clean and Accounting Aligned.

import React, { useState, useEffect, useRef, useMemo } from 'react';
import { useTranslation } from 'react-i18next';
import { 
  Plus, Loader2, AlertTriangle, CheckCircle2, ShieldAlert, 
  PartyPopper, Coffee, Timer, Trash2, Calendar, Building2, User
} from 'lucide-react';
...
const DashboardPage: React.FC = () => {
  ...
  const [isCreating, setIsLoadingCreating] = useState(false);
  ...
  const handleCreateCase = async (e: React.FormEvent) => {
    ...
    try {
      const payload: CreateCaseRequest = { 
        case_number: `C-${Date.now().toString().slice(-6)}`, 
      ...
      alert(t('error.generic', 'Ndodhi një gabim gjatë krijimit.'));
    } finally {
  ...
  const confirmDeleteCase = async () => {
    ...
    try {
      ...
      alert(t('error.caseDeleteFailed', 'Dështoi fshirja e klientit.'));
    } finally {
      setIsDeletingCase(false);
    }
  };

  const inputClasses = "glass-input w-full px-5 py-3.5 rounded-2xl text-sm transition-all placeholder:text-text-secondary/40 border border-white/5 focus:border-primary-start/50";
  const labelClasses = "block text-[10px] font-black text-primary-start uppercase tracking-[0.2em] mb-3 ml-1";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-8 h-full flex flex-col relative">
            ...
            <div className={`p-6 sm:p-8 bg-gradient-to-br ${theme.style}`}>
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-8">
                ...
                >
                    ...
                    {t('briefing.view_calendar', 'SHIKO KALENDARIN')}
                </button>
              </div>
            </div>
          </motion.div>
      ...
      <div className="flex flex-row justify-between items-end mb-8 gap-4 px-2">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
            {t('dashboard.mainTitle', 'Pasqyra e Klientëve')}
          </h1>
          <p className="text-xs sm:text-sm text-gray-500 font-bold mt-1 uppercase tracking-widest opacity-60">
            {t('dashboard.subtitle', 'Menaxhoni Klientët dhe Kompanitë Tuaja.')}
          </p>
        </div>
        <button 
        ...
        >
          ...
          <span className="hidden sm:inline">{t('dashboard.newCase', 'Klient i Ri')}</span>
        </button>
      </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-8">
          {cases.length === 0 ? (
             <div className="glass-panel flex flex-col items-center justify-center py-24 text-gray-600">
                ...
                <p className="font-black uppercase tracking-widest text-xs italic">Nuk u gjetën klientë aktivë.</p>
             </div>
          ) : (
      ...
      <AnimatePresence>
        {showCreateModal && (
          <div className="fixed inset-0 bg-background-dark/80 backdrop-blur-xl flex items-center justify-center z-[100] p-4">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95, y: 20 }} 
              animate={{ opacity: 1, scale: 1, y: 0 }} 
              exit={{ opacity: 0, scale: 0.95 }} 
              className="glass-high w-full max-w-lg p-8 sm:p-10 rounded-[3rem] shadow-2xl shadow-black/60 border border-white/5"
            >
              <h2 className="text-2xl font-black text-white mb-10 tracking-tight uppercase flex items-center gap-3">
                <Building2 className="text-primary-start" />
                {t('dashboard.createCaseTitle', 'Krijo Klient të Ri')}
              </h2>
              <form onSubmit={handleCreateCase} className="space-y-8">
                <div className="space-y-4">
                  <label className={labelClasses}>BIZNESI / KOMPANIA</label>
                  <input 
                    required 
                    placeholder={t('dashboard.clientTitlePlaceholder', 'Emri i Biznesit (Psh: ABC Sh.p.k)')} 
                ...
                <div className="pt-8 border-t border-white/10 space-y-6">
                  <label className={labelClasses}>
                    <span className="flex items-center gap-2">
                        <User size={14} />
                        PËRFAQËSUESI / KONTAKTI
                    </span>
                  </label>
                  <input 
                    required 
                    placeholder={t('dashboard.clientName', 'Emri dhe Mbiemri i Pronarit')} 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input 
                      placeholder={t('dashboard.clientEmail', 'Email i Biznesit')} 
                      value={newCaseData.clientEmail} 
                      onChange={(e) => setNewCaseData(p => ({...p, clientEmail: e.target.value}))} 
                      className={inputClasses} 
                    />
                    <input 
                      placeholder={t('dashboard.clientPhone', 'Numri i Telefonit')} 
                ...
                <div className="flex justify-between items-center mt-12 pt-4">
                  <button 
                    ...
                    className="px-8 py-4 font-black text-gray-500 hover:text-white transition-all text-[10px] uppercase tracking-widest"
                  >
                  ...
                  <button 
                    type="submit" 
                    disabled={isCreating} 
                    className="glass-button px-12 h-14 rounded-2xl flex items-center justify-center gap-3 active:scale-95 text-[10px] font-black uppercase tracking-widest disabled:opacity-50 shadow-lg shadow-primary-start/20"
                  >
        ...
        {caseToDeleteId && (
          <div className="fixed inset-0 bg-background-dark/80 backdrop-blur-xl flex items-center justify-center z-[110] p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="glass-high w-full max-w-md p-10 rounded-[3rem] shadow-2xl text-center border border-accent-start/30">
              ...
              <h2 className="text-2xl font-black text-white mb-3 uppercase tracking-tight">{t('caseDelete.confirmTitle', 'Fshij Klientin?')}</h2>
              <p className="text-gray-400 text-sm mb-10 leading-relaxed italic font-medium">{t('caseDelete.confirmMessage', 'Ky veprim është i pakthyeshëm. Të gjitha të dhënat dhe dokumentet e këtij biznesi do të fshihen.')}</p>
              <div className="flex justify-center gap-5">
                <button type="button" onClick={() => setCaseToDeleteId(null)} className="glass-button flex-1 h-14 rounded-2xl text-[10px] uppercase tracking-widest font-black">{t('general.cancel', 'Anulo')}</button>
